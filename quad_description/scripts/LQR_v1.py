...
class QuadrotorLQRNode(Node):
    ...
    def __init__(self):
        ...
        R = np.diag([
            1.0,   # F  (large  → cheap to vary thrust)
            1.0,    # τ_roll
            1.0,    # τ_pitch
            0.001,    # τ_yaw
        ])
        ...
        # #   State: [x, y, z, φ, θ, ψ, ẋ, ẏ, ż, φ̇, θ̇, ψ̇]─────────# ─────────────
        #     400.0,  400.0,  400.0,   # position  x, y, z     — penalise position error
        #     100.0,  100.0,  100.0,    # attitude  φ, θ, ψ
        #     16.0,   16.0,   16.0,    # velocity  ẋ, ẏ, ż
        #     4.0,    4.0,    4.0,    # ang-rate  φ̇, θ̇, ψ̇
        #    ψ,   ẋ, #  ẏ,  ż,   φ̇,  θ̇,  ψ̇]
        #
        #  • Positi# on (x, y, z):
        # #     - High weight → drone snaps tightly to the traject# ory setpoint
        #     - # z slightly higher because alti# tude is critical for safety
#         #  • Attitude (φ, θ): high to keep the drone level against wind gusts
        #  • Yaw (ψ): moderate (trajectory rarely demands yaw changes)
        #  • Velocity (ẋ, ẏ, ż): moderate-high to damp post-disturbance oscillations
        #  • Angular rates (φ̇, θ̇, ψ̇): moderate to avoid sluggish attitude response
        #
        # ── R: Input penalty ─────────────────────────────────────────────────
        #   Input: [F_total, τ_roll, τ_pitch, τ_yaw]
        #
        #  • Lower R → controller can apply larger corrections → faster disturbance
        #    rejection (wind resistance) and tighter trajectory following.
        #  • F_total very small (0.005): thrust can change freely (main authority).
        #  • Torques small but larger than F: allow fast attitude corrections without
        #    saturation on the smaller moment arms.

        # Q = np.diag([
        #     200.0, 200.0, 300.0,   # position  x, y, z
        #      80.0,  80.0,  30.0,   # attitude  φ, θ, ψ
        #       8.0,   8.0,  10.0,   # velocity  ẋ, ẏ, ż
        #       5.0,   5.0,   3.0,   # ang-rate  φ̇, θ̇, ψ̇
        # ])
        # R = np.diag([0.005, 0.05, 0.05, 0.2])
