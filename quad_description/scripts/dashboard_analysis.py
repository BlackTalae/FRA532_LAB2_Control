...
def build_robustness():
    ...
    return fig, axes_r, info

def update_robustness(axes_r: dict, laps: list, info_text):
    if not laps:
        return
    ...
    # Cumulative per lap
    total_cumulative_error = 0  # Initialize total cumulative error for all laps
    total_cx, total_cy, total_cz = 0, 0, 0  # Initialize sum for each axis (X, Y, Z)
    
    avg_cumulative_error = []  # List to store the average cumulative error per lap
    avg_cx, avg_cy, avg_cz = [], [], []  # List to store average for each axis per lap
    ...
    for ax_k, dkey, ylabel, title, tc in [
        ...
        for i, lap in enumerate(laps):
            ...
            # Accumulate the cumulative error for each axis
            if ax_k == "cx_l":
                total_cx += np.sum(c_)
                avg_cx.append(np.mean(c_))  # Average for X axis
            elif ax_k == "cy_l":
                total_cy += np.sum(c_)
                avg_cy.append(np.mean(c_))  # Average for Y axis
            elif ax_k == "cz_l":
                total_cz += np.sum(c_)
                avg_cz.append(np.mean(c_))  # Average for Z axis

            total_cumulative_error += np.sum(c_)  # Add to total cumulative error
            avg_cumulative_error.append(np.mean(c_))  # Average for total error
    ...
    # Display cumulative error for all laps and the sum of each axis in info_text
    info_text.set_text(
        f'  ▶  CUMULATIVE ERRORS ——'
        f'  Total Error: {total_cumulative_error:.4f} m·s'  # Total cumulative error for all laps
        f'  |  X Error: {total_cx:.4f} m·s'
        f'  |  Y Error: {total_cy:.4f} m·s'
        f'  |  Z Error: {total_cz:.4f} m·s'
        f'  |  Average Total Error: {np.mean(avg_cumulative_error):.4f} m·s'
        f'  |  Avg X Error: {np.mean(avg_cx):.4f} m·s'
        f'  |  Avg Y Error: {np.mean(avg_cy):.4f} m·s'
        f'  |  Avg Z Error: {np.mean(avg_cz):.4f} m·s'
    )
