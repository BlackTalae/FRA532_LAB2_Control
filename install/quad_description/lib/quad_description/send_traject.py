...
class TrajectoryNode(Node):
    ...
    def __init__(self, waypoints):
        ...
        self._pub_static_path = self.create_publisher(Path, '/show_path', 10)
    ...
    def _build_static_path(self) -> Path:
        """
        Build a complete static path, which is the full trajectory.
        Used to publish the entire trajectory to /static_path topic.
        """
        stamp = self.get_clock().now().to_msg()
        path_msg = Path()
        path_msg.header.stamp = stamp
        path_msg.header.frame_id = 'odom'  # or 'world' depending on your frame

        for waypoint in self._waypoints:
            x, y, z, yaw = waypoint
            pose_stamped = self._make_pose_stamped(x, y, z, yaw, stamp)
...
def main(args=None):
    ...
    node._pub_static_path.publish(node._build_static_path())
    try:
        rclpy.spin(node)
